...
export function buttonChoice(label: string, shortcut: string, value: string, data: string, disabled: boolean, selected = false, strongest = false): string { return `<button class="action-button option-card ${selected ? "is-selected" : ""} ${strongest ? "is-strongest" : ""}" ${data} ${disabled ? "disabled" : ""}${selected ? ' aria-current="true"' : ""}><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong>${strongest ? "<em>Strongest</em>" : ""}${shortcutHint(shortcut)}</button>`; }
