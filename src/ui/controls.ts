...
export function primaryButton(id: string, label: string, shortcut: string, disabled = false): string { return `<button class="control control--primary action-button primary-action" id="${escapeHtml(id)}" ${disabled ? "disabled" : ""}><span>${escapeHtml(label)}</span>${shortcutHint(shortcut)}</button>`; }
export function quietButton(id: string, label: string, shortcut: string): string { return `<button class="control control--quiet action-button quiet" id="${escapeHtml(id)}"><span>${escapeHtml(label)}</span>${shortcutHint(shortcut)}</button>`; }
...
export function surface(tag: "aside" | "section", classes: string, label: string, content: string): string { return `<${tag} class="surface panel ${escapeHtml(classes)}" aria-label="${escapeHtml(label)}">${content}</${tag}>`; }
